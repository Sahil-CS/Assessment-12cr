...
def make_hard_questions():
    ...
    while len(used) < 3:
        ...
        if p not in used:
            quiz.append({"question": "Differentiate: " + p["expr"], "answer": p["answer"]})
            used.append(p)
    return quiz
...
menu = tk.Frame(app, bg="#ccf2cc")
menu.pack(pady=30)

tk.Label(menu, text="Welcome to the Math Quiz!", font=title_font, bg="#ccf2cc").pack(pady=10)
tk.Label(menu, text="Your name:", font=custom_font, bg="#ccf2cc").pack()
name_input = tk.Entry(menu, font=normal_font, width=30)
name_input.pack(pady=5)

tk.Label(menu, text="Choose difficulty:", font=custom_font, bg="#ccf2cc").pack(pady=(20, 5))
...
btn_frame = tk.Frame(menu, bg="#ccf2cc")
btn_frame.pack(pady=20)
tk.Button(btn_frame, text="Start", font=normal_font, width=15,
          bg="green", fg="#ccf2cc", command=start_quiz).grid(row=0, column=0, padx=10)
tk.Button(btn_frame, text="Quit", font=normal_font, width=15,
          bg="red", fg="#ccf2cc", command=quit_game).grid(row=0, column=1, padx=10)

quiz_frame = tk.Frame(app, bg="#ccf2cc")

question_text = tk.Label(quiz_frame, text="", font=normal_font, wraplength=500, bg="#ccf2cc")
...
tk.Button(quiz_frame, text="Submit Answer", font=normal_font, width=20,
          bg="blue", fg="#ccf2cc", command=check).pack(pady=10)
tk.Button(quiz_frame, text="Quit", font=normal_font, width=20,
          bg="red", fg="#ccf2cc", command=quit_game).pack(pady=5)
